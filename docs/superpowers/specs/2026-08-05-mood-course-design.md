...
- radius는 두 단계다 (Apple Quarter 참조에서 채택한 둥근 기하학). **면** — 선택된 목차 행, 지도 컨테이너, 카드류 — 는 `--radius-card: 16px`. **알약** — 버튼, 마커, 배지 — 는 `9999px`. 레일과 헤어라인 구분선은 직선으로 남는다. 행이 둥글어지면 행 사이 헤어라인은 제거한다(둥근 면에 붙은 밑줄은 어색하다). 현재 흩어져 있는 7 · 12 · 14 · 18px은 이 두 단계로 정리한다
- 사진 원리 — *장소가 색을 갖고, 인터페이스는 갖지 않는다* (Apple Quarter). 무채색 UI는 유지하되 포스터·장소 사진이 화면의 유일한 색이 되도록 사진 비중을 키운다. 적용은 기존 화면 재도색 계획에서 한다
